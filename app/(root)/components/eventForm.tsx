...
interface EventFormProps {
  newEvent: {
    id?: string; // Thêm id để xác định sự kiện nếu cập nhật/xoá
    title: string;
    start: Date;
    end: Date;
    description: string;
...
const EventForm: React.FC<EventFormProps> = ({
  ...
  const handleUpdate = async () => {
    try {
      const res = await newRequest.put(`/api/v1/event/${newEvent.id}`, newEvent, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      alert('Event updated successfully!');
      setShowEventForm(false); // Đóng form sau khi cập nhật thành công
    } catch (error) {
      console.error('Error updating event:', error);
    }
  };

  // Hàm xử lý xoá sự kiện
  const handleDelete = async () => {
    try {
      await newRequest.delete(`/api/v1/event/${newEvent.id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      alert('Event deleted successfully!');
      setShowEventForm(false); // Đóng form sau khi xoá thành công
    } catch (error) {
